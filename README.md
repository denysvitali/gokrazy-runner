...
`:8443` over HTTPS using a **per-device** self-signed certificate at
`/perm/ssl/gokrazy-web.{pem,key.pem}` (generated by `pkg/tlsconfig` on
first boot, ECDSA P-256, 1-year validity, auto-renewed within 30 days
of expiry). The shared rootfs cert at `/etc/ssl/gokrazy-web.*` is only
used as a one-boot fallback before the per-device cert has been
generated, and is never preferred over `/perm/ssl/...`. The server
falls back to plain HTTP on `:8080` if no certificate is readable (or
if `WEBUI_LISTEN_HTTP_ONLY` is set). Once the device is online, point
a browser at `https://<device>:8443/`.
