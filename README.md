...
```mermaid
flowchart TB
    subgraph root["gokrazy root (squashfs, read-only)"]
        direction TB
        subgraph ours["This repo"]
            permInit["perm-init<br/><i>one-shot: grow GPT p4, mke2fs, reboot</i>"]
            runnerInit["runner-init<br/><i>long-lived: supervises podman</i>"]
            webui["runner-webui<br/><i>HTTPS UI on :8443</i>"]
            tsInit["tailscale-init<br/><i>one-shot: tailscale up</i>"]
            usbInit["usbdev-init<br/><i>udev stand-in, /dev/bus/usb</i>"]
        end
        subgraph upstream["Upstream packages"]
            ts["tailscaled + tailscale"]
            podman["gokrazy/podman + iptables + nsenter"]
            bg["gokrazy/breakglass<br/><i>emergency SSH</i>"]
            misc["serial-busybox, fbstatus"]
        end
    end

    subgraph perm["/perm (ext4, persistent)"]
        direction TB
        envFile["runner.env<br/>URL=… NAME=… LABELS=… IMAGE=…"]
        tokFile["runner.token<br/><i>one-shot GH registration token</i>"]
        dataDir["runner-data/<br/><i>container's /home/runner</i>"]
        bgKeys["breakglass/authorized_keys"]
        tsKey["tailscale.authkey<br/><i>chmod 0600</i>"]
        tsState["tailscale/<br/><i>tailscaled state</i>"]
        pwFile["gokr-pw.txt<br/><i>update + UI password</i>"]
        sslCerts["ssl/gokrazy-web.{pem,key.pem}<br/><i>generated by gokrazy</i>"]
        otaHist["ota-install-history.json"]
    end

    container["ghcr.io/actions/actions-runner:latest<br/><i>config.sh on first boot, then run.sh</i>"]

    runnerInit -- "reads" --> envFile
    runnerInit -- "reads, deletes after use" --> tokFile
    runnerInit -- "podman run, mounts" --> dataDir
    runnerInit -- "podman run --entrypoint /bin/bash" --> container
    container -- "/home/runner" --> dataDir

    webui -- "reads/writes" --> envFile
    webui -- "writes" --> tokFile
    webui -- "reads/writes" --> bgKeys
    webui -- "reads/writes" --> tsKey
    webui -- "reads/writes" --> pwFile
    webui -- "reads (TLS)" --> sslCerts
    webui -- "appends" --> otaHist

    tsInit -- "reads" --> tsKey
    tsInit -- "tailscale up" --> ts
    ts -- "state" --> tsState

    bg -- "reads" --> bgKeys
    permInit -- "creates / mounts" --> perm
```
